                    ...
                    foreach (var tasks in taskList)
                    {
                        ...
                        using (DDContext context = new DDContext())
                        {
                            ...
                            if (taskList.Count == 1 && taskList.IndexOf(tasks) == 0)  //未选人
                            {
                                ...
                                {
                                    //推送OA消息
                                    SentCommonMsg(dic["PeopleId"].ToString(), string.Format("您有一条待审批的流程(流水号:{0})，请及时登入华数信息管理系统进行审批。", TaskId), tasks.ApplyMan, tasks.Remark, null);
                                }
                            }
                            else  //有选人
                            {
                                if (taskList.IndexOf(tasks) > 0 && !IsRepeat && IsTs)
                                {
                                    ...
                                    //推送OA消息
                                    SentCommonMsg(tasks.ApplyManId, string.Format("您有一条待审批的流程(流水号:{0})，请及时登入华数信息管理系统进行审批。", TaskId), taskNew.ApplyMan, taskNew.Remark, null);
                                }
                                //对最后一条重复数据进行寻人推送
            ...
            try
            {
                ...
                else
                {
                    ...
                    new form{ key="审批时间：",value=DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")},
                                    ...
                                    title = string.Format("您有一条被退回的流程(流水号:{0})，请及时登入华数信息管理系统进行查阅。", tasks.TaskId),
